...
PARAM_NAMES = [
    ...
    "insole_lateral_bias",
    "insole_heel_lift",
    "insole_forefoot_lift",
    "insole_ridge_amp",
    "insole_pocket_amp",
    *[
        f"insole_h_{ix:02d}_{iy:02d}"
        for ix in range(6)
        for iy in range(3)
    ],
]
...
def regression_metrics(y_true: np.ndarray, y_pred: np.ndarray, base_ids: np.ndarray) -> dict[str, Any]:
    ...
    out: dict[str, Any] = {
        ...
        "pressure_nrmse": nrmse(y_true, y_pred),
        "pressure_rmse": rmse(y_true, y_pred),
        "pressure_mae": mae(y_true, y_pred),
        "pressure_maxae": maxae(y_true, y_pred),
    ...
    for base_id in sorted(set(base_ids.tolist())):
        ...
        basewise[f"base_{base_id:02d}"] = {
            ...
            "pressure_nrmse": nrmse(y_true[mask], y_pred[mask]),
            "pressure_rmse": rmse(y_true[mask], y_pred[mask]),
            "pressure_mae": mae(y_true[mask], y_pred[mask]),
            "n": int(np.sum(mask)),
        }
...
def flatten_regression_baselines(regression: dict[str, Any]) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    baselines = regression.get("baselines", {})
    for split_name, split_payload in baselines.items():
        for baseline_name, metrics in split_payload.items():
            ...
            for base_name, base_metrics in metrics.get("by_base", {}).items():
                base_row = {
                    "split": split_name,
                    "baseline": baseline_name,
                    "base": base_name,
...
def main() -> None:
    ...
    write_csv(args.out_dir / "regression_baselines_long.csv", flatten_regression_baselines(regression))
