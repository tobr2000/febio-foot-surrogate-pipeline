...
class FFNContactPressureModel(nn.Module):
    """Pointwise baseline: parameters + contact-face coordinates -> pressure."""

    def __init__(self, param_dim: int, hidden_dim: int = 128, layers: int = 4, dropout: float = 0.05):
        super().__init__()
        self.net = mlp(2 + param_dim, hidden_dim, 1, layers, dropout)
...
def build_model(param_dim: int, cfg: dict) -> FFNContactPressureModel:
    mcfg = cfg["model"]
