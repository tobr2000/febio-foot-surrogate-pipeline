...
def normalized_physical_components(
    ...
    pressure = pointwise_loss(p, y, lcfg)
    ...
    mean_weight = float(lcfg.get("pressure_mean_weight", 0.0))
    if mean_weight > 0.0:
        true_mean = y_raw.mean(dim=1)
        pred_mean = p_pos.mean(dim=1)
        mean_scale = torch.clamp(true_mean.abs(), min=0.05)
        components["pressure_mean"] = torch.mean(((pred_mean - true_mean) / mean_scale).square()) * mean_weight

    std_weight = float(lcfg.get("pressure_std_weight", 0.0))
    if std_weight > 0.0:
        true_std = y_raw.std(dim=1, unbiased=False)
        pred_std = p_pos.std(dim=1, unbiased=False)
        std_scale = torch.clamp(true_std.abs(), min=0.02)
        components["pressure_std"] = torch.mean(((pred_std - true_std) / std_scale).square()) * std_weight

    max_weight = float(lcfg.get("pressure_max_weight", 0.0))
    if max_weight > 0.0:
        components["pressure_max"] = torch.mean(((pred_peak - true_peak) / peak_scale).square()) * max_weight

    cop_weight = float(lcfg.get("center_of_pressure_weight", 0.0))
    coord_source = batch.get("contact_geom_x", batch.get("contact_x"))
    if cop_weight > 0.0 and coord_source is not None and coord_source.shape[-1] >= 2:
        coord_dim = min(3, coord_source.shape[-1])
        coords = coord_source[..., :coord_dim]
        true_w = torch.relu(y_raw)
        pred_w = p_pos
        true_center = torch.sum(coords * true_w, dim=1) / torch.clamp(torch.sum(true_w, dim=1), min=1.0e-6)
        pred_center = torch.sum(coords * pred_w, dim=1) / torch.clamp(torch.sum(pred_w, dim=1), min=1.0e-6)
        components["center_of_pressure"] = torch.mean((pred_center - true_center).square()) * cop_weight
    ...
    if topk_weight > 0.0:
        ...
        components["topk_pressure"] = pointwise_loss(top_pred, top_true, lcfg, prefix="topk_pressure") * topk_weight

    regional_reaction_weight = float(lcfg.get("regional_reaction_weight", 0.0))
    regional_peak_weight = float(lcfg.get("regional_peak_weight", 0.0))
    region = contact_region_features(batch)
    if region is not None and (regional_reaction_weight > 0.0 or regional_peak_weight > 0.0):
    ...
    if "residual_scale" in pred:
        components["residual_scale_l2"] = torch.mean(pred["residual_scale"].square()) * float(
            lcfg.get("residual_scale_l2_weight", 0.0)
        )
    return components
...
def pointwise_loss(
    ...
    """Dense supervised loss with an optional robust mode for sharp contact peaks."""
    loss_kind = str(lcfg.get(f"{prefix}_loss", lcfg.get("pressure_loss", "mse"))).lower()
    if loss_kind in {"smooth_l1", "huber"}:
        beta = float(lcfg.get(f"{prefix}_loss_beta", lcfg.get("pressure_loss_beta", 0.5)))
        return F.smooth_l1_loss(pred, target, beta=beta)
    if loss_kind in {"l1", "mae"}:
...
def _component_cadence_active(name: str, cfg: dict[str, Any], global_step: int | None) -> bool:
    ...
    if name in {"pressure", "reaction", "peak", "nonnegative_pressure", "topk_pressure", "regional_reaction", "regional_peak", "reaction_head", "peak_head", "pressure_shape_anchor", "pressure_residual_l2", "residual_scale_l2", "pressure_mean", "pressure_std", "pressure_max", "center_of_pressure"}:
        every = 1
...
def pinn_component_multiplier(name: str, cfg: dict[str, Any], epoch: int, global_step: int | None = None) -> float:
    ...
    if name in {"element_aux", "node_displacement_aux"}:
        return float(pcfg.get("data_aux_max_multiplier", 0.35)) * ramp_multiplier(
            epoch,
            int(pcfg.get("data_aux_start_epoch", 1)),
            int(pcfg.get("data_aux_ramp_epochs", 1)),
        )
    if name.startswith("history_"):
        return float(pcfg.get("history_max_multiplier", 0.20)) * ramp_multiplier(
            epoch,
            int(pcfg.get("history_start_epoch", pcfg.get("physics_start_epoch", 10))),
            int(pcfg.get("history_ramp_epochs", pcfg.get("physics_ramp_epochs", 20))),
        )
    if name in {"contact_projection", "contact_complementarity"}:
        return float(pcfg.get("contact_max_multiplier", 0.15)) * ramp_multiplier(
            epoch,
            int(pcfg.get("contact_start_epoch", pcfg.get("physics_start_epoch", 10))),
            int(pcfg.get("contact_ramp_epochs", pcfg.get("physics_ramp_epochs", 20))),
        )
    return float(pcfg.get("physics_max_multiplier", 0.20)) * ramp_multiplier(
        epoch,
...
def apply_env_overrides(cfg: dict[str, Any]) -> None:
    """Allow Slurm jobs to run focused ablations without creating many config files."""
    training_float_env = {
    ...
    for env_name, cfg_name in training_float_env.items():
        value = _env_float(env_name)
        if value is not None:
            cfg["training"][cfg_name] = value
    ...
    for env_name, cfg_name in training_int_env.items():
        value = _env_int(env_name)
        if value is not None:
            cfg["training"][cfg_name] = value

    scheduler = _env_str("LR_SCHEDULER")
    if scheduler:
    ...
    for env_name, cfg_name in model_int_env.items():
        value = _env_int(env_name)
        if value is not None:
            cfg["model"][cfg_name] = value
    ...
    for env_name, cfg_name in model_float_env.items():
        value = _env_float(env_name)
        if value is not None:
            cfg["model"][cfg_name] = value
    ...
    loss_str_env = {
        ...
        "TOPK_PRESSURE_LOSS": "topk_pressure_loss",
    }
    for env_name, cfg_name in loss_str_env.items():
        value = _env_str(env_name)
        if value:
            cfg["loss"][cfg_name] = value

    loss_float_env = {
        "PRESSURE_LOSS_BETA": "pressure_loss_beta",
        "TOPK_PRESSURE_LOSS_BETA": "topk_pressure_loss_beta",
        "PRESSURE_WEIGHT": "pressure_weight",
        "REACTION_WEIGHT": "reaction_weight",
        "PEAK_WEIGHT": "peak_weight",
        "TOPK_PRESSURE_WEIGHT": "topk_pressure_weight",
        "TOPK_PRESSURE_FRACTION": "topk_pressure_fraction",
        "PRESSURE_MEAN_WEIGHT": "pressure_mean_weight",
        "PRESSURE_STD_WEIGHT": "pressure_std_weight",
        "PRESSURE_MAX_WEIGHT": "pressure_max_weight",
        "CENTER_OF_PRESSURE_WEIGHT": "center_of_pressure_weight",
        "NONNEGATIVE_PRESSURE_WEIGHT": "nonnegative_pressure_weight",
        "PRESSURE_RESIDUAL_L2_WEIGHT": "pressure_residual_l2_weight",
        "RESIDUAL_SCALE_L2_WEIGHT": "residual_scale_l2_weight",
    }
    for env_name, cfg_name in loss_float_env.items():
        value = _env_float(env_name)
        if value is not None:
            cfg["loss"][cfg_name] = value

    curriculum_int_env = {
        "PINN_DATA_AUX_START_EPOCH": "data_aux_start_epoch",
        "PINN_DATA_AUX_RAMP_EPOCHS": "data_aux_ramp_epochs",
        "PINN_PHYSICS_START_EPOCH": "physics_start_epoch",
        "PINN_PHYSICS_RAMP_EPOCHS": "physics_ramp_epochs",
        "PINN_HISTORY_START_EPOCH": "history_start_epoch",
        "PINN_HISTORY_RAMP_EPOCHS": "history_ramp_epochs",
        "PINN_CONTACT_START_EPOCH": "contact_start_epoch",
        "PINN_CONTACT_RAMP_EPOCHS": "contact_ramp_epochs",
        "PINN_DATA_AUX_EVERY_STEPS": "data_aux_every_steps",
        "PINN_PHYSICS_EVERY_STEPS": "physics_every_steps",
        "PINN_HISTORY_EVERY_STEPS": "history_every_steps",
        "PINN_CONTACT_EVERY_STEPS": "contact_every_steps",
    }
    for env_name, cfg_name in curriculum_int_env.items():
        value = _env_int(env_name)
        if value is not None:
            cfg.setdefault("pinn_curriculum", {})[cfg_name] = value

    curriculum_float_env = {
        "PINN_DATA_AUX_MAX_MULTIPLIER": "data_aux_max_multiplier",
        "PINN_PHYSICS_MAX_MULTIPLIER": "physics_max_multiplier",
        "PINN_HISTORY_MAX_MULTIPLIER": "history_max_multiplier",
        "PINN_CONTACT_MAX_MULTIPLIER": "contact_max_multiplier",
    }
    for env_name, cfg_name in curriculum_float_env.items():
        value = _env_float(env_name)
        if value is not None:
            cfg.setdefault("pinn_curriculum", {})[cfg_name] = value
...
def run_training(model_name: str, build_model_fn, argv: list[str] | None = None) -> None:
    ...
    robalrs_enabled_models = robalrs_cfg.get("enabled_models", ["pinn"])
    if isinstance(robalrs_enabled_models, str):
        robalrs_enabled_models = [robalrs_enabled_models]
    robalrs_enabled_set = {str(name).lower() for name in robalrs_enabled_models}
    robalrs_enabled = bool(robalrs_cfg.get("enabled", True)) and model_name.lower() in robalrs_enabled_set
    loss_balancer = RobALRSLossBalancer(
        enabled=robalrs_enabled,
    ...
    print(f"[STARTUP] robalrs_enabled={loss_balancer.enabled} for model={model_name}", flush=True)
    ema_model = None
    ...
    for epoch in range(1, int(cfg["training"]["epochs"]) + 1):
        ...
        for batch in tqdm(train_loader, desc=f"{model_name}/train", leave=False):
            ...
            if model_name == "pinn":
                ...
                base_loss, base_parts, components = base_pressure_loss(pred, batch, cfg, stats)
                if loss_balancer.enabled:
                    loss, parts = loss_balancer.combine(components)
                else:
                    loss, parts = base_loss, base_parts
            if device.type == "cuda":
                torch.cuda.synchronize()
